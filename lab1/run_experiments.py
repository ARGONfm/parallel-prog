...
print(f"Логических ядер: {logical_cores}")

# Для эксперимента используем физические ядра
if physical_cores >= 8:
    CORE_COUNTS = [1, 2, 4, 8]
else:
    CORE_COUNTS = [1, 2, 4]  # для 4-ядерных систем

print(f"Будем тестировать ядра: {CORE_COUNTS}")
...
print(f"Параметры: размеры={MATRIX_SIZES}, потоки={THREAD_COUNTS}, ядра={CORE_COUNTS}")
...
for n in MATRIX_SIZES:
    for threads in THREAD_COUNTS:
        for cores in CORE_COUNTS:
            if threads > cores:
                print(f"  Пропускаем: threads={threads} > cores={cores}")
                continue
                
            for run_num in range(1, NUM_RUNS + 1):
                current_run += 1
                print(f"[{current_run}/{total_runs}]")
                exec_time = run_experiment(n, threads, cores)

                if exec_time is not None:
                    with open(LOG_FILE, 'a') as f:
                        f.write(f"{n},{threads},{cores},{run_num},{exec_time:.6f}\n")
                else:
                    with open(LOG_FILE, 'a') as f:
                        f.write(f"{n},{threads},{cores},{run_num},ERROR\n")
                time.sleep(1)

print(f"\nЭксперименты завершены! Результаты в {LOG_FILE}")